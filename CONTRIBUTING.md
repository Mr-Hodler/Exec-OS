# Contributing to Exec-OS

Thanks for improving Exec-OS. This is a skill/plugin suite, so most contributions are edits to Markdown rules and the config schema, not code.
...
- `.exec-os-config.yml` - the shared config every skill reads first. Extend the schema here when a skill needs new settings, and document the addition in SETUP.md.
...
- **Safety is non-negotiable** in workspace-ops: never weaken a safeguard (never delete, archive + snapshot + undo log + confirmation + blacklist). Adding safeguards is welcome; removing one is not.
- Every skill **reads `.exec-os-config.yml` first** and degrades gracefully when a path or platform is unreachable. Preserve that contract.
...
For workspace-ops, always test against a disposable copy first and confirm no file is ever deleted, only archived.
