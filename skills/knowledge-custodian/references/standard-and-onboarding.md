...
4. **Leave a seam for the data room.** For a company that will have a data room, do not recreate the canonical data room folders here; leave a clear place for `dataroom-ops` Bootstrap to lay them down, so the two skills do not duplicate or fight over structure.
5. **Report.** List exactly what was created and what was intentionally left to other skills (the data room to dataroom-ops).

## Boundary with dataroom-ops Bootstrap
