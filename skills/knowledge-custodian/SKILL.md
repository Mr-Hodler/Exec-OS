...
- **To the future HR/Org skill.** Architect produces the inferred functional map and a friction list grounded in the information topology. The HR/Org skill consumes that evidence to own people and team-structure decisions and to help scale the team (roles, team boundaries, vision), the way diligence-ops consumes the data room. knowledge-custodian provides the topological evidence; HR/Org provides the org design. They share the inferred functional map and never overlap: one owns information architecture, the other owns team architecture.
