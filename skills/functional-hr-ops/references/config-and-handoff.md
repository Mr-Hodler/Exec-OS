...
Begin by reading `.knowledge-ops-config.yml` (prefer `.knowledge-ops-config.local.yml` if present). functional-hr-ops uses:
...
- **knowledge-custodian (bidirectional).** Consumes the de-facto map and frictions; produces `Organization-Model.md`. knowledge-custodian reads the model to place files and shape information architecture. functional-hr-ops owns team architecture, knowledge-custodian owns information architecture; the model is the contract. Details: `organization-model.md`.
