...
Begin every run by reading `.exec-os-config.yml` (prefer `.exec-os-config.local.yml` if present). The fields workspace-ops depends on, and how it treats them:

- `workspace_ops.scope_paths` - the only locations it may look in. No work happens outside them.
- `workspace_ops.never_touch` - the absolute blacklist, loaded and checked before every read or operation. If missing or unreadable, **stop and ask**; never guess a safety boundary.
- `workspace_ops.archive_folder_name`, `snapshot_folder_name`, `confirm_batch_over` - the archive, snapshot, and confirmation settings used by Advise plans and v1.1 Execute.
...
- **To investor-ops.** A clean substrate makes Sync's classification reliable. When investor-ops reports a chaotic or ambiguous source, that is the cue to run a Scan plus Advise here on that source. workspace-ops organizes the raw files; investor-ops then syncs the finals into the data room. The two never reorganize the same thing.
- **Onboarding vs Bootstrap.** Onboarding scaffolds the company's general working structure and leaves the data room area for investor-ops Bootstrap. One owner per structure.
- **To investor-ops.** Indirect: by keeping the substrate clean and consistent, workspace-ops ensures the data room investor-ops packages was built on solid ground. No direct call.
