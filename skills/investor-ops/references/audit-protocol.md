...
- Flag mis-tiered items (employee PII or litigation tagged `public`, a pitch deck tagged `restricted` for no reason). Mis-tiering is a leakage risk for investor-ops, so treat a too-open tag on sensitive content as a blocker.
