# Board and investor delivery

The board pack and investor letter are authored and assembled by Founder-OS `narrative-assets-ops`. investor-ops delivers them: it files the finished pack in the data room, audience-gates it, logs distribution, and manages the recurring cadence. It never authors, assembles, or re-derives content. If the pack is missing or stale, it routes to narrative-assets-ops. The templates below describe what the delivered pack contains (the spec narrative-assets-ops fills), for gating and completeness checks.

## Sources (pull, do not re-derive)

- **Metrics:** `09_Traction_&_Metrics` in the data room and the Founder-OS `metrics-dashboard` dashboard. Use the dashboard as the single source of the numbers; cite it.
...
If a number is not in the room or the dashboard, do not invent it; flag it as missing and route to investor-ops or Founder-OS.
...
If board/investor reporting later outgrows being a mode here (high volume, complex recurring cadence), it is the clean candidate to graduate into a dedicated `reporting-ops` skill; until then it lives in investor-ops.
